...
/**
 * Core service to capture Guardrails approval requests and initialize steps (actor-based).
 */
class ActorApprovalService
{
    /**
    ...
    public static function capture($model, array $dirty, string $event = 'updating'): ApprovalRequest
    {
        ...
        $request->actor_id = optional($actor)->id;
        ...
        // Build steps from model-defined rules or a default one (prefer actorApprovalFlow)
        if (method_exists($model, 'actorApprovalFlow')) {
            $flow = (array) $model->actorApprovalFlow($dirty, $event);
        } elseif (method_exists($model, 'humanApprovalFlow')) {
            $flow = (array) $model->humanApprovalFlow($dirty, $event);
        } else {
            $flow = [];
        }
        if (empty($flow)) {
